...
<article id="post-<?php the_ID(); ?>" <?php post_class('relative flex flex-col sm:flex-row bg-neutral-900 border border-neutral-800 rounded-lg overflow-hidden transition-all duration-300 hover:border-red-600/50 mb-4 sm:mb-6'); ?>>
    ...
    <div class="relative w-full sm:w-2/5 lg:w-1/3 shrink-0 bg-neutral-800 md:max-w-64">
        ...
        <div class="mb-2 sm:mb-3 flex flex-wrap gap-2">
            ...
            <?php elseif (has_category()) : ?>
                ...
                foreach ($categories as $category) {
                    echo '<a href="' . esc_url(get_category_link($category->term_id)) . '" class="text-xs font-bold uppercase bg-neutral-800 text-red-600 hover:bg-red-600 hover:text-neutral-100 px-3 py-1 transition-colors duration-300">' . esc_html($category->name) . '</a>';
                }
                ?>
            <?php endif; ?>
        </div>
